...
WITH LatestScrapeDates AS (
  SELECT
    property_code,
    Max(dlt_scrape_date) AS dlt_scrape_date
  FROM {{ ref("fct_properties") }}
  GROUP BY property_code
),

properties_removed as (
SELECT
  ls.property_code,
  ls.dlt_scrape_date,
  fpu.price,
...
WHERE ls.dlt_scrape_date != (SELECT Max(dlt_scrape_date) FROM {{ ref("fct_properties") }}))
,
join_duplicates AS (
  SELECT
    pr.*,
    dpa.duplicate_group_id
  FROM properties_removed pr
  LEFT JOIN {{ ref("dim_property_duplicates") }}  dpa ON pr.property_code = dpa.duplicate_property_code
),
live_duplicates AS (
  SELECT
    duplicate_group_id,
