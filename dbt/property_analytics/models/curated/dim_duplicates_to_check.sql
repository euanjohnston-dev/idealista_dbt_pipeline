...
distinct_properties AS (
  ...
  SELECT * FROM {{ ref("dim_property_distinct") }} fp
...
duplicate_pairs AS (
  ...
  FROM
    property_duplicates p1
  JOIN
    property_duplicates p2
  ON
...
LEFT JOIN distinct_properties pdi 
...
LEFT JOIN property_duplicates dpd1 ON d.first_duplicate = dpd1.property_code
LEFT JOIN property_duplicates dpd2 ON d.second_duplicate = dpd2.property_code
WHERE 
  d.distance < 1
